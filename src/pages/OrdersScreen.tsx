...
const OrderDetailsModal = ({ 
      ...
      >
        ...
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(safeOrderStatus)}`}>
        ...
        {order.market && (
          <div className="mb-3">
            <p className="text-sm font-medium text-blue-800">{order.market.market_name || 'Unknown Market'}</p>
            <p className="text-xs text-gray-600">{order.market.address || 'No address'}</p>
          </div>
        )}
        ...
        {order.deliveryAddress && (
          <div className="mb-3">
            <p className="text-sm font-medium">Deliver to: {order.deliveryAddress.city || 'Unknown City'}</p>
            <p className="text-xs text-gray-600">Zipcode: {order.deliveryAddress.zipcode || 'N/A'}</p>
          </div>
        )}
        ...
        {order.user && (
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-600">{order.user.phone || 'No phone'}</p>
          </div>
        )}
        ...
        {order.otp && (
          <div className="mb-3 bg-yellow-50 p-2 rounded text-center">
            <p className="text-xs text-yellow-700 font-medium">OTP: {order.otp}</p>
          </div>
        )}
  ...
  const OrdersScreen = () => {
    ...
    const activeOrders = buckets.PLACED.length + 
      ...
      <div className="p-4 h-screen bg-[#e8f4f8]">
        <div className="mb-4">
          ...
          <div className="flex gap-6 text-sm text-gray-600">
            ...
            <span>Active Orders: <strong>{activeOrders}</strong></span>
            <span>Delivered: <strong>{buckets.DELIVERED.length}</strong></span>
            <span>Cancelled: <strong>{buckets.CANCELLED.length}</strong></span>
